// ─────────────────────────────────────────────────────────────────────────────
// server/src/db.ts — SQLite via @libsql/client (Turso-compatible)
//
// Remote Turso:    DB_URL=libsql://your-db.turso.io  DB_AUTH_TOKEN=your-token
// Local file:      DB_PATH=/data/lands.db  (or leave unset → ./data/lands.db)
// In-memory test:  DB_PATH=:memory:
// ─────────────────────────────────────────────────────────────────────────────
...
function getClient(): Client {
  ...
  // Remote Turso takes priority when DB_URL looks like a libsql:// or https:// URL
  const dbUrl = process.env.DB_URL?.trim();
  if (dbUrl && (dbUrl.startsWith('libsql://') || dbUrl.startsWith('https://'))) {
    _client = createClient({ url: dbUrl, authToken: process.env.DB_AUTH_TOKEN });
    return _client;
  }

  // Local / in-memory SQLite via DB_PATH
  const rawPath = process.env.DB_PATH?.trim();
  const url = rawPath
    ? (rawPath === ':memory:' ? ':memory:' : `file:${rawPath}`)
    : 'file:data/lands.db';
...
export const BUILTIN_PACKS: SkinPackRow[] = [
  {
    ...
    price_cents: 0, preview_url: '/cards/skins/default/preview.svg',
...
export async function createUser(id: string, username: string, passcode_hash: string): Promise<UserRow> {
...
export async function recordGameResult(result: GameResultInsert): Promise<[number, number]> {
  ...
  const delta0 = Math.round(kFactor(p0) * (score0 - eloExpected(p0.elo, p1.elo)));
  const delta1 = Math.round(kFactor(p1) * (score1 - eloExpected(p1.elo, p0.elo)));
  ...
  await Promise.all([
    getClient().execute({
      sql: `UPDATE users SET
              ...
              wins_five_kind = wins_five_kind + ?, wins_rainbow = wins_rainbow + ?,
              games_for_k = MIN(games_for_k + 1, 9999), last_seen = ?
            WHERE id = ?`,
      args: [newElo0, score0===1?1:0, score0===0?1:0, score0===0.5?1:0,
             score0===1?1:0, score0===1?1:0,
             score0===1&&isFiveKind?1:0, score0===1&&isRainbow?1:0, now, p0id],
    }),
    getClient().execute({
      sql: `UPDATE users SET
              elo = ?, wins = wins + ?, losses = losses + ?, draws = draws + ?,
              ...
              games_for_k = MIN(games_for_k + 1, 9999), last_seen = ?
            WHERE id = ?`,
      args: [newElo1, score1===1?1:0, score1===0?1:0, score1===0.5?1:0,
             score1===1?1:0, score1===1?1:0,
             score1===1&&isFiveKind?1:0, score1===1&&isRainbow?1:0, now, p1id],
    }),
    getClient().execute({
      sql: `INSERT INTO game_results (id, p0_id, p1_id, winner_index, win_reason, turn_count, elo_delta_p0, elo_delta_p1, played_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
