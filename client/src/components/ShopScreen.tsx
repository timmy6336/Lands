...
async function openUrl(url: string) {
  if (Capacitor.isNativePlatform()) {
    // Dynamically import to avoid bundling issues when Capacitor Browser is absent
    try {
...
export function ShopScreen({ auth, serverUrl, onBack, onProfileUpdated }: Props) {
  ...
  function cardUrl(packId: string, color: string) {
    return `/cards/skins/${packId}/${color}.svg`;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {!auth.profile && (
          <span style={{ marginLeft: 'auto', color: 'var(--muted2)', fontSize: '0.8rem' }}>Sign in to unlock</span>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.5rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, alignItems: 'start' }}>
          ...
          {!shop && !loadErr && (
            <p style={{ color: 'var(--muted)', textAlign: 'center', marginTop: '4rem', gridColumn: '1/-1' }}>Loading…</p>
          )}
          ...
          {shop && visiblePacks.map(pack => {
            ...
            return (
              ...
              >
                ...
                {isExpanded && (
                  <div style={{
                    borderTop: '1px solid var(--border)', padding: '0.7rem 0.85rem',
                    ...
                    {CARD_COLORS.map(color => (
                      <div key={color} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                        <img
                          ...
                          alt={color}
                          style={{ width: 44, height: 62, borderRadius: 5, objectFit: 'cover', border: '1px solid var(--border)', boxShadow: '0 1px 4px rgba(0,0,0,0.18)' }}
